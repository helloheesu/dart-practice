...
가상의 학생 성적 분석 프로그램을 만들어봅니다.

- **파일로부터 데이터 읽어오기**와 **클래스 구성**은 앞서 제시된 필수 1, 2번 문항과 동일하게 진행합니다.
- 프로그램 실행 후, 사용자 입력을 받아 **메뉴 방식**으로 기능을 선택할 수 있도록 만듭니다.
- 사용자가 `1`을 입력하면, **우수생(가장 높은 평균 점수를 가진 학생)의 정보**를 출력합니다.
- 사용자가 `2`를 입력하면, **전체 학생들의 평균 점수**를 계산하여 출력합니다.
...
- 평균 점수가 가장 높은 학생을 찾아 이름과 평균 점수를 출력합니다.
- 출력 예시: `우수생: 홍길동 (평균 점수: 92.5)`

[ 힌트 ]

- `where()`나 `fold()`를 이용해 학생별 점수 합계를 구할 수 있습니다.
- 학생별 점수 평균을 구한 뒤, `reduce()`나 `sort()`를 사용하면 최고 점수를 가진 학생을 찾을 수 있습니다.
...
- 전체 학생들의 점수를 모두 합산합니다.
- 합산한 점수를 전체 점수 개수로 나누어 평균을 계산합니다.
- 출력 예시: `전체 평균 점수: 84.3`

[ 힌트 ]

- 전체 점수 리스트를 `map()`으로 추출한 뒤 `reduce()` 또는 `fold()`를 사용하여 합계를 계산할 수 있습니다.
- 평균은 `(총합 / 개수)`로 구합니다.
- 소수점 자리수를 제한하려면 `toStringAsFixed(2)`를 사용하세요.
